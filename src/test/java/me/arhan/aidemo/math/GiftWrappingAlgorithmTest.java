...
/**
 * Unit tests for the Gift Wrapping (Jarvis March) convex hull algorithm.
 */
class GiftWrappingAlgorithmTest {
    ...
    void testCalculateOrientation() {
        ...
        assertEquals(2, GiftWrappingAlgorithm.calculateOrientation(p1, p2, p3));
        
        // Points in clockwise order
        assertEquals(1, GiftWrappingAlgorithm.calculateOrientation(p1, p3, p2));
        
        // Collinear points
        Point p4 = new Point(2, 0);
        assertEquals(0, GiftWrappingAlgorithm.calculateOrientation(p1, p4, p2));
    }
    ...
    void testCalculateOrientationWithNullPoints() {
        ...
        assertThrows(IllegalArgumentException.class, () -> GiftWrappingAlgorithm.calculateOrientation(null, p1, p2));
        assertThrows(IllegalArgumentException.class, () -> GiftWrappingAlgorithm.calculateOrientation(p1, null, p2));
        assertThrows(IllegalArgumentException.class, () -> GiftWrappingAlgorithm.calculateOrientation(p1, p2, null));
    }
    ...
    void testConvexHullOfSquare() {
        ...
        List<Point> hull = GiftWrappingAlgorithm.run(points);
    ...
    void testConvexHullWithFewerThan3Points() {
        ...
        List<Point> hull1 = GiftWrappingAlgorithm.run(points1);
        ...
        List<Point> hull2 = GiftWrappingAlgorithm.run(points2);
    ...
    void testConvexHullWithNullInput() {
        assertThrows(IllegalArgumentException.class, () -> GiftWrappingAlgorithm.run(null));
        ...
        assertThrows(IllegalArgumentException.class, () -> GiftWrappingAlgorithm.run(pointsWithNull));
    }
    ...
    private boolean containsPoint(List<Point> points, int x, int y) {
        return points.stream().anyMatch(p -> p.getX() == x && p.getY() == y);
    }
}